...
def datafr():
    df= pd.read_csv(r"C:\Users\ss996\OneDrive\Desktop\Projects\P3-Airbnb/airbnb_cleaned_data.csv")
    return df
...
if select == "Data Exploration":
    ...
    with tab1:
        ...
        with col1:
            ...
            fig_bar= px.bar(df_bar, x='property_type', y= "price", title= "PRICE FOR PROPERTY_TYPES",hover_data=["number_of_reviews","review_scores"],
                            color_discrete_sequence=px.colors.sequential.Brwnyl, width=600, height=500)
        ...
        with col1:
            ...
            barmode='group',color_discrete_sequence=px.colors.sequential.amp_r, width=600, height=500)
        ...
        with col2:
            ...
            barmode='group',color_discrete_sequence=px.colors.sequential.Darkmint_r, width= 600, height= 500)
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv(r"C:\Users\ss996\OneDrive\Desktop\Projects\P3-Airbnb\airbnb_cleaned_data.csv")
            return df_a
        ...
        with col1:
            ...
            df_a_sunb_30= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_30",width=600,
                                      height=500,title="Availability_30",color_discrete_sequence=px.colors.sequential.haline)
        ...
        with col2:
            ...
            df_a_sunb_60= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_60",
                                      width=600,height=500,title="Availability_60",color_discrete_sequence=px.colors.sequential.algae_r)
        ...
        with col1:
            ...
            df_a_sunb_90= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_90",
                                      width=600,height=500,title="Availability_90",color_discrete_sequence=px.colors.sequential.Greys)
        ...
        with col2:
            ...
            df_a_sunb_365= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_365",
                                       width=600,height=500,title="Availability_365",color_discrete_sequence=px.colors.sequential.YlOrRd_r)
        ...
        df_mul_bar_a= pd.DataFrame(df3_a.groupby("host_response_time")[["availability_30","availability_60","availability_90",
                                                                        "availability_365","price"]].sum())
        ...
        barmode='group',color_discrete_sequence=px.colors.sequential.speed,width=1200)
    ...
    with tab3:
        ...
        val_sel= st.radio("Select the Price Range",[str(df2_l['price'].min())+' '+str('to')+' '+str(differ_max_min*0.30 + df2_l['price'].min())+' '+
                                                    str("(30% of the Value)"),
                                                    
                                                    str(differ_max_min*0.30 + df2_l['price'].min())+' '+str('to')+' '+str(differ_max_min*0.60 +
                                                     df2_l['price'].min())+' '+str("(30% to 60% of the Value)"),

                                                    str(differ_max_min*0.60 + df2_l['price'].min())+' '+str('to')+' '+str(df2_l['price'].max())+
                                                    ' '+str("(60% to 100% of the Value)")])
        ...
        # List of columns to keep for correlation calculation
        columns_to_keep = [
            "_id", "minimum_nights", "maximum_nights", 'accommodates', "bedrooms", "beds",
            "number_of_reviews", "bathrooms", "price", 'extra_people', 'guests_included',
            "review_scores", "host_id", "host_listings_count", "host_total_listings_count",
            "cleaning_fee","availability_30", "availability_60", "availability_90", "availability_365"
        ]

        # Select only the specified columns
        df_val_sel = df_val_sel[columns_to_keep]

        # Calculate the correlation matrix
        df_val_sel_corr = df_val_sel.corr()
        ...
        fig_1= px.bar(df_val_sel_gr, x="accommodates", y= ["cleaning_fee","bedrooms","beds"], title="ACCOMMODATES",
                    hover_data= "extra_people", barmode='group', color_discrete_sequence=px.colors.sequential.Turbo_r,width=1000)
        st.plotly_chart(fig_1)
        
        
        # room_ty_l= st.selectbox("Select the Room_Type_l", df_val_sel["room_type"])
        ...
        #             hover_data= ["name","host_name","market"], barmode='group',orientation='h', color_discrete_sequence=px.colors.sequential.Rainbow_r,
        #             width=1000)
